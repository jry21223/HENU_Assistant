...
def test_agent_public_result_redacts_sensitive_nested_fields() -> None:
    result = sanitize_public_result(
        {
            "auth": {"token": "TOKEN", "CASTGC": "TGT", "JSESSIONID": "SESSION"},
            "data": [{"password": "secret", "id": "43"}],
        }
    )

    assert result["auth"]["token"] == "<redacted>"
    assert result["auth"]["CASTGC"] == "<redacted>"
    assert result["auth"]["JSESSIONID"] == "<redacted>"
    assert result["data"][0]["password"] == "<redacted>"
    assert result["data"][0]["id"] == "43"
