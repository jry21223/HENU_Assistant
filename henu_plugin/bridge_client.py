...
配置推送、实时状态、账号密码登录。HTTPS 认证服务端，AES-GCM 保护载荷。
客户端拒绝重复响应；请求关联协议升级需要桥端共同支持。

配置（环境变量，与 HENU_MASTER_KEY 同一 .env）：
  HENU_BRIDGE_URL      必须为 HTTPS 地址
...
_SEEN_RESPONSES: dict[str, float] = {}
...
def bridge_settings() -> dict[str, str] | None:
    ...
    try:
        for line in ENV_FILE.read_text(encoding='utf-8').splitlines():
            ...
            if sep and key.strip() in {'HENU_BRIDGE_URL', 'HENU_BRIDGE_SECRET'}:
                parts = shlex.split(raw, comments=True)
                values[key.strip()] = parts[0] if parts else ''
    except FileNotFoundError:
        pass
    except (OSError, ValueError) as exc:
        raise BridgeError('桥配置文件无法读取或格式错误') from exc
    url = os.environ.get("HENU_BRIDGE_URL", values.get('HENU_BRIDGE_URL', '')).strip().rstrip("/")
    secret = os.environ.get("HENU_BRIDGE_SECRET", values.get('HENU_BRIDGE_SECRET', '')).strip()
    if not url or not secret:
        return None
    parsed = urlsplit(url)
    if parsed.scheme != 'https' or not parsed.hostname or parsed.username or parsed.password or parsed.query or parsed.fragment:
        raise BridgeError('桥地址必须是无认证参数的 HTTPS 地址')
...
def _open(secret: str, path: str, body: bytes) -> dict[str, Any]:
    try:
        ...
        raise BridgeError("桥响应信封不合法") from exc
    ...
    if not isinstance(result, dict):
        raise BridgeError('桥响应必须是 JSON 对象')
    fingerprint = hashlib.sha256(body).hexdigest()
    now = time.time()
    with _REPLAY_LOCK:
        for key, deadline in list(_SEEN_RESPONSES.items()):
            if deadline < now:
                del _SEEN_RESPONSES[key]
        if fingerprint in _SEEN_RESPONSES or len(_SEEN_RESPONSES) >= 10000:
            raise BridgeError('桥响应重复或重放窗口已满')
        _SEEN_RESPONSES[fingerprint] = ts + TS_WINDOW_SECONDS
    return result
